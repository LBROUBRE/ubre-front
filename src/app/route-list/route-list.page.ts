...
export class RouteListPage implements OnInit {
  ...
  getRouteList(){ //Obtenemos todas las rutas asociadas a una solicitud
    const URL = "http://localhost:8000/movility/requestsfront/" + this.ID_solicitud + ""; // url/id_de_solicitud_que_recibimos_de_tab1
    this.httpClient.get(URL).subscribe(res => {
      if (res == null)
        return
      var res_origin_lat = res[0]["origin"][0];
      var res_origin_lng = res[0]["origin"][1];
      var res_destination_lat = res[0]["destination"][0];
      var res_destination_lng = res[0]["destination"][1];
      this.routes[0] = {
        routeID: "",
        polylineID: res[0]["polylineID"],
        origin: res[0]["origin"],
        destination: res[0]["destination"],
        timeToDest: res[0]["timeToDest"],
        price: res[0]["price"],
      };
      fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${res_origin_lat}&lon=${res_origin_lng}`)
      .then(res => res.json())
      .then(myJson => {
        this.routes[0]["origin"] = myJson["display_name"]
      });
      fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${res_destination_lat}&lon=${res_destination_lng}`)
      .then(res => res.json())
      .then(myJson => {
        this.routes[0]["destination"] = myJson["display_name"]
      }); 
    })
  }
